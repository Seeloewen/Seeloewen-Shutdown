...
Public Class frmMain
    'General variables
    Private Const version As String = "1.10.0"
    Private Const verDate As String = "19.02.2024"
    Public Const settingsVersion As Integer = 1
    Private loadedSettingsVersion As Integer
    Private actionRunning As Boolean = False
    Public closeResult As ClosingResult = ClosingResult.Close
    ...
    Public rtbLog As RichTextBox = New RichTextBox() 'This needs to be reworked!
    ...
    Private Sub frmMain_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        ...
        If actionRunning = True And My.Settings.ShowCloseWarning = True Then
            ...
            If closeResult = ClosingResult.Cancel Then
                e.Cancel = True
            End If
        End If
    End Sub
    ...
    Private Sub tmrShutdown_Tick(sender As Object, e As EventArgs) Handles tmrShutdown.Tick
        ...
        If timeSpan.TotalMilliseconds > 0 Then
            If timeSpan.TotalHours > 24 Then
                lblTimeRemainingContent.Text = timeSpan.ToString("dd\:hh\:mm\:ss")
            Else
                lblTimeRemainingContent.Text = timeSpan.ToString("hh\:mm\:ss")
            End If
        End If
    End Sub
    ...
    Private Sub rbtnZeitpunkt_CheckedChanged(sender As Object, e As EventArgs) Handles rbtnPointInTime.CheckedChanged
        ...
        If rbtnIn.Checked = True Then
            If String.IsNullOrEmpty(tbTime.Text) = False Then
                lblSelectedTimeContent.Text = String.Format("In {0} {1}", tbTime.Text, cbxIn.Text)
            Else
                If language = "English" Then
                    lblSelectedTimeContent.Text = "No time selected"
                ElseIf language = "German" Then
                    lblSelectedTimeContent.Text = "Keine Zeit gewählt"
                End If
            End If
        ElseIf rbtnPointInTime.Checked = True Then
            lblSelectedTimeContent.Text = dtpDate.Text
        End If
    End Sub
    ...
    Private Sub rbtnShutdown_CheckedChanged(sender As Object, e As EventArgs) Handles rbtnShutdown.CheckedChanged
        ...
        If rbtnShutdown.Checked = True Then
            If language = "English" Then
                lblSelectedActionContent.Text = "Shutdown"
            ElseIf language = "German" Then
                lblSelectedActionContent.Text = "Herunterfahren"
            End If
        ElseIf rbtnRestart.Checked = True Then
            If language = "English" Then
                lblSelectedActionContent.Text = "Restart"
            ElseIf language = "German" Then
                lblSelectedActionContent.Text = "Neustarten"
            End If
        End If
    End Sub
    ...
    Private Sub _TimeRemaining_TextChanged(sender As Object, e As EventArgs) Handles lblTimeRemainingContent.TextChanged
        'Change text in Minimalistic View if time remaining text in main window gets updated
        frmMinimalisticView.lblTimerShutdown.Text = lblTimeRemainingContent.Text
    End Sub
    ...
    Private Sub LoadLanguage()
        ...
        If language = "German" Then
            ...
            lblSelectedActionContent.Text = "Keine Aktion gewählt"
            lblSelectedTimeContent.Text = "Keine Zeit gewählt"
        ...
        ElseIf language = "English" Then
            ...
            lblSelectedActionContent.Text = "No action selected"
            lblSelectedTimeContent.Text = "No time selected"
    ...
    Private Sub LoadDesign()
        ...
        If design = "Dark" Then
            ...
            lblSelectedActionContent.BackColor = Color.FromArgb(41, 41, 41)
            lblSelectedTimeContent.BackColor = Color.FromArgb(41, 41, 41)
            lblSelectedAction.ForeColor = Color.White
            lblSelectedTime.ForeColor = Color.White
            lblSelectedActionContent.ForeColor = Color.White
            lblSelectedTimeContent.ForeColor = Color.White
            lblAction.ForeColor = Color.White
            lblTime.ForeColor = Color.White
            gbLastAction.ForeColor = Color.White
            lblLastAction.ForeColor = Color.White
            lblLastActionContent.ForeColor = Color.White
            lblLastTime.ForeColor = Color.White
            lblLastTimeContent.ForeColor = Color.White
            lblStartedOn.ForeColor = Color.White
            lblStartedOnContent.ForeColor = Color.White
            ...
            lblRunningActionContent.BackColor = Color.FromArgb(25, 25, 25)
            lblRunningTimeContent.BackColor = Color.FromArgb(25, 25, 25)
            lblTimeRemainingContent.BackColor = Color.FromArgb(25, 25, 25)
            lblScheduledAction.ForeColor = Color.White
            lblScheduledTime.ForeColor = Color.White
            lblTimeRemaining.ForeColor = Color.White
            lblRunningActionContent.ForeColor = Color.White
            lblRunningTimeContent.ForeColor = Color.White
            lblTimeRemainingContent.ForeColor = Color.White
    ...
    Private Sub CheckForRunningAction()
        ...
        If timeDifference.TotalMilliseconds < 0 Then
            selectedTime = Convert.ToDateTime(My.Settings.LastTime)
            SetupGrayBox((selectedTime - Now).TotalSeconds)
            CallLastAction()
    ...
    Public Sub EditRunningAction(actionNew As String, changeType As String, change As String, showNotification As Boolean)
        ...
        Dim timeSpan As TimeSpan = newTime - Now
        Dim shutdownTime As Integer = Math.Ceiling(timeSpan.TotalSeconds)

        'Check if new time is valid
        If shutdownTime > 31535999 Then
            If language = "German" Then
        ...
        Else
            ...
            'Start action
            Process.Start("shutdown", String.Format("{0} -t {1}", action, shutdownTime)) 'Start the action
            ...
            If showNotification = True Then
                ...
                End If
            End If
            WriteToLog(String.Format("Edited the current action (Time: {0}, Action: {1})", shutdownTime, action), "Info")
        End If
    End Sub
    ...
    Private Sub SetLastAction()
        'Set action so software can recognize that an action is running when being restarted
        My.Settings.LastAction = lblRunningActionContent.Text
        My.Settings.LastTime = selectedTime
        My.Settings.LastDate = DateTime.Now

        'Display last action
        My.Settings.LastActionDisplay = lblRunningActionContent.Text
        My.Settings.LastTimeDisplay = selectedTime
        My.Settings.LastDateDisplay = DateTime.Now

        'Add action to Action History file
        If My.Settings.EnableActionHistory Then
            Try
                My.Computer.FileSystem.WriteAllText(actionHistoryFile, String.Format("{0};{1};{2}" + vbNewLine, lblRunningActionContent.Text, selectedTime, DateTime.Now), True)
            Catch ex As Exception
    ...
    Private Sub CallLastAction()
        ...
        Dim timeSpan As TimeSpan = selectedTime - Now
        Dim shutdownTime As Integer = Math.Ceiling(timeSpan.TotalSeconds)
        countDownFrom = TimeSpan.FromSeconds(shutdownTime)
        lblRunningTimeContent.Text = My.Settings.LastTime

        tmrShutdown.Interval = 100
        targetDT = Now.Add(countDownFrom)
        tmrShutdown.Start()

        lblRunningActionContent.Text = My.Settings.LastAction
        WriteToLog(String.Format("Called last action (CountDownFrom: {0}, ShutdownTime.Text: {1}, _RunningTime.text: {2})", countDownFrom, shutdownTime, lblRunningTimeContent.Text), "Info")
    End Sub

    Private Sub SetupGrayBox(shutdownTime As String)
        'Display running action
        If language = "English" Then
            If rbtnShutdown.Checked Then
                lblRunningActionContent.Text = "Shutdown"
            ElseIf rbtnRestart.Checked Then
                lblRunningActionContent.Text = "Restart"
            End If
        ElseIf language = "German" Then
            If rbtnShutdown.Checked Then
                lblRunningActionContent.Text = "Herunterfahren"
            ElseIf rbtnRestart.Checked Then
                lblRunningActionContent.Text = "Neustarten"
            End If
        End If
        ...
        If rbtnIn.Checked Then
            ...
            selectedTime = DateTime.Now.AddSeconds(shutdownTime)
            lblRunningTimeContent.Text = selectedTime
        ElseIf rbtnPointInTime.Checked Then
            selectedTime = dtpDate.Value
            countDownFrom = TimeSpan.FromSeconds(Convert.ToInt32(shutdownTime))
            lblRunningTimeContent.Text = dtpDate.Text
        End If
        ...
        WriteToLog(String.Format("Set gray box up (CountDownFrom: {0}, Shutdowntime.Text: {1}, _RunningTime.Text: {2})", countDownFrom.ToString, shutdownTime, lblRunningTimeContent.Text), "Info")
    End Sub
    ...
    Private Sub StartAction()
        ...
        If rbtnIn.Checked = True Then
            ...
            Else 'Set Shutdown type
                ...
                'Run action
                If shutdownTime > maxTime Then 'Show error if shutdown time is greater than the max number allowed
                    If language = "German" Then
                        MsgBox(String.Format("Die maximale Zeitangabe für die ausgewählte Option beträgt {0}", maxTime), MsgBoxStyle.Critical, "Fehler")
                    ElseIf language = "English" Then
                        MsgBox(String.Format("The maximum time for the chosen option is {0}", maxTime), MsgBoxStyle.Critical, "Error")
                    End If
                ElseIf shutdownTime = 0 Then 'Show error if shutdown time is zero
                    If language = "German" Then
                ...
                Else
                    ...
                    WriteToLog(String.Format("Started action (Time: {0}, Action: {1}, Type: In...)", shutdownTime, action), "Info")
                End If
            End If
        ...
        ElseIf rbtnPointInTime.Checked = True Then 'Set action using "Point in Time"
            ...
            dtpDate.CustomFormat = "dd.MM.yyyy HH:mm:ss"
            Dim timeSpan As TimeSpan = dtpDate.Value.Date + dtpDate.Value.TimeOfDay - DateTime.Now
            Dim shutdownTime As Integer = Math.Ceiling(timeSpan.TotalSeconds)
            ...
            Else
                ...
                WriteToLog(String.Format("Started action (Time: {0}, Action: {1}, Type: PointInTime)", shutdownTime, action), "Info")
            End If
        End If
    End Sub
        ...
        If My.Settings.LoadProfileByDefault = True Then
            ...
            If String.IsNullOrEmpty(My.Settings.DefaultProfile) = False Then
                If My.Computer.FileSystem.FileExists(String.Format("{0}{1}.txt", profileDirectory, My.Settings.DefaultProfile)) Then
                    ...
                    Dim defaultProfile As String = My.Settings.DefaultProfile
                    frmLoadProfileFrom.InitializeLoadingProfile(defaultProfile, False)
                Else
                    If language = "English" Then
    ...
    Private Sub btnHamburger_MouseDown(sender As Object, e As MouseEventArgs) Handles btnHamburger.MouseDown
        If design = "Dark" Then
            btnHamburger.BackgroundImage = My.Resources.hamburger_click_dark
        ElseIf design = "Light" Then
            btnHamburger.BackgroundImage = My.Resources.hamburger_click
        End If
    End Sub

    Private Sub btnHamburger_MouseEnter(sender As Object, e As EventArgs) Handles btnHamburger.MouseEnter
        If design = "Dark" Then
            btnHamburger.BackgroundImage = My.Resources.hamburger_hover_dark
        ElseIf design = "Light" Then
            btnHamburger.BackgroundImage = My.Resources.hamburger_hover
        End If
    End Sub

    Private Sub btnHamburger_MouseLeave(sender As Object, e As EventArgs) Handles btnHamburger.MouseLeave
        btnHamburger.BackgroundImage = My.Resources.hamburger
    End Sub
